...
#     changeAcc(account["user1"]["user"],account["user1"]["pass"])
    ...
    #Case 3: MUA VIP 1
    cheatTimeRemain(account["user1"]["id"], 0) #cheat non-vip
    reloadLobby()
    back_to_lobby()
    check_buy_vip(account["user1"]["id"], vip_pack["vip.pack_1"]["name"]) #mua vip 1
    to_table()
    check_item() #check item trong ban choi
    back_to_lobby()
    reportBuyVip1(data)
    #Case 4: MUA VIP 2
    check_buy_vip(account["user1"]["id"], vip_pack["vip.pack_2"]["name"])
    to_table()
    check_item() #check item trong ban choi
    back_to_lobby()
    reportBuyVip2(data)
    #Case 5: Mua vip 3
    closeAllEvent()
    check_buy_vip(account["user1"]["id"], vip_pack["vip.pack_3"]["name"])
    to_table()
    check_item() #check item trong ban choi
    back_to_lobby()
    ...
    cheatTimeRemain(account["user1"]["id"], 1)
    checkMoGUIGH()
    back_to_lobby()
    reportExpiredVip(data)
    #Case11: Check het han trong ban choi
    api_postDoFunction(account["user1"]["id"], "CHEAT_TIME_REMAIN_VIP", [10])
    to_table()
    time.sleep(3)
    check_item()
    ...
    checkMoGUIVip()
    check_buy_vip(account["user1"]["id"], vip_pack["vip.pack_1"]["name"])
...
def DB():
    ...
    api_changeTimeServer(convertDayTimeToMili(time_db[0]))
    check_login()
    #2. đăng kí thường
    register()     
    play_tutorial()
    ...
    log_out()
    time.sleep(3)
    api_changeTimeServer(convertDayTimeToMili(time_db[1]))
    time.sleep(3)
    log_in()
    time.sleep(3)
    #5. Nhận bonus lần 2
    ...
    api_changeTimeServer(convertDayTimeToMili(time_db[2]))
    time.sleep(1)
    reloadLobby()
    time.sleep(10)
    ...
    log_out()
    api_changeTimeServer(convertDayTimeToMili(time_db[3]))
    log_in()
    ...
    clear()
    close_Gui_daily()
    #16 Log out-> Login sau 24h 
    time.sleep(2)
    log_out()
    api_changeTimeServer(convertDayTimeToMili(time_db[7]))
    time.sleep(3)
    log_in()
    time.sleep(3)
    ...
    api_changeTimeServer(convertDayTimeToMili(time_db[8]))
    reloadLobby()
    time.sleep(5)
